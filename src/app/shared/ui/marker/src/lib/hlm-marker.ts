...
const markerVariants = cva(
  "text-muted-foreground group/marker [a]:hover:text-foreground relative flex min-h-4 w-full items-center gap-1.5 text-start text-sm [&_ng-icon:not([class*='text-'])]:text-[length:--spacing(4)] [a]:underline [a]:underline-offset-3",
  {
    variants: {
      variant: {
        default: '',
